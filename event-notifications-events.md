...
3. In the {{site.data.keyword.openpages_short}} navigation, click **Settings**.
4. In the {{site.data.keyword.en_short}} section, and click **Connect**.
5. Expand the **After connection** section.
6. In the **Event Notification instances** section, click **Event Notification service instance** and select the {{site.data.keyword.en_short}} that you created.
...
An email from {{site.data.keyword.cloud_notm}} that contains information about a {{site.data.keyword.openpages_short}} event resembles the following example:

<!-- Replace this with an example from your service-->
...
Successful events that are generated by {{site.data.keyword.openpages_short}} contain various fields that help you to identify the source and details of an event.

Event notifications from {{site.data.keyword.openpages_short}} contain only metadata properties, such as names or identifiers of resources. Sensitive data, for example API keys or passwords, are not included in generated events.
{: note}
